    ...
    data(d){
        ...
        const clustering = () => {
            const generateClusters = () => {
                const getClusterTags = () => {this.clusterTags = this.clusters.map(d=>d.key)};

                if (this.domainType[this.clusterOn] !== 'Categorical')
                    this.clusterOn = this.keys.find(key => this.domainType[key] === 'Categorical');
                if (this.clusterOn)
                    this.clusters = d3.nest().key(d => d[this.clusterOn]).entries(d);
                else
                    this.clusters = [{key: 'all', value: d}];
            ...
            console.log('sorted clusters', this.sortedClusters);
