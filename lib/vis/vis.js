...
class DataUtils {
    ...
    clusterBy(key){
        if(this.domainType[key] === 'Categorical'){
            const clusters = {};
            this.domain[key].forEach( value => {
                clusters[value] = this.d.filter(d => d[key] === value);
            });
            return clusters;
        }
        return `${key} is not a categorical attribute`
    }
}
class ParallelBundling extends Visualization{
    ...
    constructor(parentElement, settings){
        ...
        this.lineFunction = d => {
            ...
            let BOX1 = 5;
            let BOX2 = 20;
            p.moveTo(this.x(this.keys[0]), this.y[this.keys[0]](d[this.keys[0]])); // Inicializa o path
            this.keys.forEach((key, index)=>{

                if(index < this.keys.length-1){
                    let nextKey = this.keys[index+1];
                    console.log(nextKey);
                    // Saída
                    ...
                    y = this.domainType[key] === 'Categorical' ? this.y[key](this.modes[key][d[this.clusterOn]]):this.y[key](this.means[key][d[this.clusterOn]]);
                    p.lineTo(x,y);

                    // Chegada
                    x = this.x(nextKey) - this.band(BOX2);
                    y = this.domainType[nextKey] === 'Categorical' ?
                        this.y[nextKey](this.modes[nextKey][d[this.clusterOn]])  :
                        this.y[nextKey](this.means[nextKey][d[this.clusterOn]]);
                    p.lineTo(x,y);
        ...
        // this.lineFunction = d => {
        //     return d3.svg.line()(_.flatten(this.keys.map( key => {
        //         let path = [[this.x(key), this.y[key](d[key])]];
        //         if (this.keys[this.keys.length-1] !== key) {
        //             // path.push([this.x(key) + 10, this.y[key](d[key])]);
        //         }
        //         return path;
        //     }), true));
        // };
    ...
    data(d){
        ...
        this.clusters = utils.clusterBy('Dimension 2');
        this.clusterKeys = Object.keys(this.clusters);
        this.colorOf.domain(this.clusterKeys);
        this.means = {};
        this.variances = {};
        this.modes = {};
        this.keys.forEach(key =>{
            if(this.domainType[key] !== 'Categorical') {
                this.means[key] = {};
                this.variances[key] = {};
                this.clusterKeys.forEach(clusterKey => {
                    let pluck = _.pluck(this.clusters[clusterKey], key);
    ...
    redraw(){
        ...
        this.foreground.selectAll("path.data")
            ...
            .attr("d", this.lineFunction)
            // .style("stroke", this.settings.color)
            .style("stroke", d=> this.colorScheme[this.clusterKeys.indexOf(d['Dimension 2'])])
            .on("mouseover", (d,i) =>{ this.event.datamouseover(d,i); })
            .on("mouseout", (d,i) =>{ this.event.datamouseout(d,i); })
            .on("click", (d,i) =>{ this.event.dataclick(d,i); });
        this.foreground.selectAll("path.data")
