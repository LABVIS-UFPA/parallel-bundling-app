...
class ParallelBundling extends Visualization{
    ...
    constructor(parentElement, settings){
        ...
        //TODO: reescrever lineFunction da implementação anterior perfeitamente igual
        this.baklineFunction = (d, dataIndex) => {
            let path = new Path();
            let x,y,x1,y1,x2,y2;
            let BOX1 = 1;
            let BOX2 = 20;
            ...
            keysExceptLast.forEach((key, keyIndex)=>{
                    ...
                    const setStartPointInContinuousAxis = () => {
                        x = this.x(key);
                        y = this.y[key](d[key]);
                        path.moveTo(x, y);
                    };
                    const setStartPointInCategoricalAxis = () => {
                        const ordination = this.clusters.find(c => c.key === cluster).values.filter(entry=>{
                        ...
                        const createDisplacementScale = () => {
                            for (let tag of this.clusterTags) {
                                ...
                                auxiliaryDisplacement += this.reservedSpaceInCategoricalAxes[tag][key][d[key]];
                            }
                            return d3.scale.linear()
                                .domain([0,quantityOfMembers])
                                .range([auxiliaryDisplacement, auxiliaryDisplacement + this.reservedSpaceInCategoricalAxes[cluster][key][d[key]]]);

                        };

                        const displacementScale = createDisplacementScale();
                        const displacement = displacementScale(relativeIndexPositionOfd);
                        x =  this.x(key);
                        y = this.y[key](d[key]) + displacement;
                        path.moveTo(x, y);
                    };
                    const computeNextCategoricalDisplacementResetingOrdination = () => {
                        const ordination = this.clusters.find(c => c.key === cluster).values.filter(entry=>{
                            return entry[key] === d[key];
                        });
                        ...
                        const displacementScale = createDisplacementScale();
                        return displacementScale(relativeIndexPositionOfd);
                    };
                    const computeNextCategoricalDisplacementWithNewOrdination = () => {

                        const ordination = this.sortedClusters[cluster][key].filter(entry=>{
                    ...
                    const lineToBox = () => {
                        ...
                        y = this.y[key](d[key]) + this.verticalPositionDisplacementOnAxis;
                        path.lineTo(x,y);
                    };
                    ...
                    const lineToClusterModeOnKey = () => {
                        ...
                        y1 = y;
                        x = this.x(key) + this.band(BOX2);
                        y = this.y[key](this.modes[key][cluster]) + this.axesSizesWithPadding[key]/2;
                        x2 = x-this.band(BOX2);
                        y2 = y;
                        path.bezierCurveTo(x1,y1, x2, y2, x, y);
                    };
                    ...
                    const lineToClusterModeOnNextKey = () => {
                        ...
                        y1 = y;
                        x = this.x(nextKey) - this.band(BOX2);
                        y = this.y[nextKey](this.modes[nextKey][cluster]) + this.axesSizesWithPadding[nextKey]/2;
                        x2 = this.x(nextKey) - this.band(50);
                        y2 = y;
                        path.bezierCurveTo(x1,y1,x2, y2, x,y);
                    };
                    ...
                    const lineToNextBox = () => {
                        ...
                        y = this.y[nextKey](d[nextKey]) + this.verticalPositionDisplacementOnAxis;
                        x2 = x - this.band(BOX1);
                        y2 = y;
                        path.bezierCurveTo(x1,y1,x2,y2,x,y);
                    };
                    ...
                    if (keyIndex === 0) {
                        if (!isCategorical(key)) setStartPointInContinuousAxis();
                        else setStartPointInCategoricalAxis();
                    }
                    ...
                    if (isCategorical(nextKey)) {
                        if(isCategorical(key))
                            this.verticalPositionDisplacementOnAxis = computeNextCategoricalDisplacementResetingOrdination();
                        else
                            this.verticalPositionDisplacementOnAxis = computeNextCategoricalDisplacementWithNewOrdination();
                    }
                    else
                        this.verticalPositionDisplacementOnAxis = 0;
        ...
        this.lineFunction = d => {
            ...
            keysExceptLast.forEach((key, keyIndex)=> {
                ...
                if (isCategorical(nextKey)) {
                    ...
                    if(!isCategorica(key)) {
                        ordination = this.sortedClusters[cluster][key].filter(entry => {
                            return entry[nextKey] === data[nextKey];
                        });
                    }
                    else {
                ...
                // TODO: MAKE SORTED CLUSTERS INCLUDE CATEGORICAL ONES (SIMPLE ORDINATION)
                let quantityOfMembersInCluster = this.clusters.find(c => c.key === cluster).values.length;
                let location_in_array_thisKey = this.sortedClusters[id][d].indexOf(data);
                let keyFactor = map(location_in_array_d, 0, quantityOfMembersInCluster, 1, -1) * FACTOR;

                let location_in_array_nextKEy = this.sortedClusters[id][n].indexOf(data);
                let nextKeyFactor = map(location_in_array_n, 0, quantityOfMembersInCluster, 1, -1) * FACTOR;
    ...
    resize(){
        ...
        data(d){
            ...
            const defineScales = () => {
                ...
                const defineReservedSpaceInCategoricalAxes = () =>{
                ...
                updateScaleDomains();
                defineYScales();
                defineReservedSpaceInCategoricalAxes();
            };
    ...
    redraw(){
        ...
        const drawLines = () => {
            ...
            const updateAllLines = () => {
                ...
                .attr("data-index", function(d,i){ return i; })
                .attr("d", this.lineFunction)
                .style("stroke", d=> ['firebrick', 'mediumseagreen', 'steelblue', 'gold', 'orange'][this.clusterTags.indexOf(d[this.clusterOn])])
                .on("mouseover", (d,i) =>{ this.event.datamouseover(d,i); })
                .on("mouseout", (d,i) =>{ this.event.datamouseout(d,i); })
                .on("click", (d,i) =>{ this.event.dataclick(d,i); });
            };
