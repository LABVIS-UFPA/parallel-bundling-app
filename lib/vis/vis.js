...
class ParallelBundling extends Visualization{
    ...
    constructor(parentElement, settings){
        super(parentElement, settings);
        this.clusterOn = 'Dimension 2';
        this.colorScheme = [ 'red', 'blue', 'gold', 'green', 'purple'];
        ...
        this.lineFunction = d => {
            ...
            keysExceptLast.forEach((key, index)=>{
                    const nextKey = this.keys[index+1];
                    let positionOnNextAxis;

                    const isCategorical = someKey => this.domainType[someKey] === "Categorical";
                    const setStartPoint = () => {
                        path.moveTo(this.x(key), this.y[key](d[key]));
                    };
                    const computePositionOnCategoricalAxis = () => {
                        const membersOfClusterInThisCategory = this.sortedClusters[cluster][key].filter(entry=>{
                           return entry[nextKey] === d[nextKey];
                        });
                        const indexOfData = membersOfClusterInThisCategory.indexOf(d);
                        const quantityOfMembers = membersOfClusterInThisCategory.length;
                        const relativePosition = quantityOfMembers - indexOfData;
                    ...
                    const lineToClusterMeanOnKey = () => {
                        x1 = x+this.band(BOX1);
                        y1 = y;
                        x = this.x(key) + this.band(BOX2);
                        y = this.y[key](this.means[key][cluster]);
                        x2 = x-this.band(BOX2);
                        y2 = y;
                        path.bezierCurveTo(x1,y1, x2, y2, x, y);
                    };
                    const lineToClusterModeOnKey = () => {
                        x1 = x+this.band(BOX1);
                        y1 = y;
                        x = this.x(key) + this.band(BOX2);
                        y = this.y[key](this.modes[key][cluster]);
                        x2 = x-this.band(BOX2);
                        y2 = y;
                        path.bezierCurveTo(x1,y1, x2, y2, x, y);
                    };
                    const lineToClusterMeanOnNextKey = () => {
                        x1 = this.x(key) + this.band(50);
                        y1 = y;
                        x = this.x(nextKey) - this.band(BOX2);
                        y = this.y[nextKey](this.means[nextKey][cluster]);
                        x2 = this.x(nextKey) - this.band(50);
                        y2 = y;
                        path.bezierCurveTo(x1,y1,x2, y2, x,y);
                    }
                    const lineToClusterModeOnNextKey = () => {
                        x1 = this.x(key) + this.band(50);
                        y1 = y;
                        x = this.x(nextKey) - this.band(BOX2);
                        y = this.y[nextKey](this.modes[nextKey][cluster]);
                        x2 = this.x(nextKey) - this.band(50);
                        y2 = y;
                        path.bezierCurveTo(x1,y1,x2, y2, x,y);
                    }
                    ...
                    const lineToNextBox = () => {
                        x1 = x + this.band(BOX2);
                        y1 = y;
                        x = this.x(nextKey) - this.band(BOX1);
                        y = this.y[nextKey](d[nextKey]);
                        x2 = x - this.band(BOX1);
                        y2 = y;
                        path.bezierCurveTo(x1,y1,x2,y2,x,y);
                    };
                    const lineToFinalPoint = () => {
                        x = this.x(nextKey);
                        y = this.y[nextKey](d[nextKey]);
                        path.lineTo(x,y);
                    };
                    ...
                    if(isCategorical(nextKey))
                        computePositionOnCategoricalAxis();

                    lineToBox();

                    if (isCategorical(key))
                        lineToClusterModeOnKey();
                    else
                        lineToClusterMeanOnKey();

                    if (isCategorical(nextKey))
                        lineToClusterModeOnNextKey();
                    else
                        lineToClusterMeanOnNextKey();
                ...
                // let pos;
                // if (this.categoricals.includes(n)) {
                //     let order = SELF.sortedClusters[id][d].filter(o => {
                //         return o[n] === data[n];
                //     });
                //
                //     let p = order.indexOf(data);
                //     let len = order.length;
                //     p = len - p;
                //     let desloc = 0;
                //     for (let k = 0; k < id; k++) {
                //         desloc += SELF.minisize[k][n][parseInt(data[n])];
                //     }
                //     let escalinha = d3.scaleLinear().domain([0, len]).range([desloc, SELF.minisize[id][n][parseInt(data[n])] + desloc]);
                //     pos = escalinha(p);
                //
                // }
    ...
    data(d){
        ...
        const clustering = () => {
            const generateClusters = () => {
                ...
                const generateClusterTags = () => {this.clusterTags = this.clusters.map(d=>d.key)};
                const clusterDataByKey = key => {this.clusters = d3.nest().key(d => d[key]).entries(d);};
                const clusterDataInOneGroup = () => {this.clusters = [{key: 'all', value: d}];}
            ...
            const generateSortedClusters = () => {
                const sortClusterMembersByKey = (clusterTag,key) => {
                ...
                this.clusterTags.forEach(clusterTag => {
                    ...
                    this.keys.forEach(key => {
                        if(this.domainType[key] !== "Categorical")
                            this.sortedClusters[clusterTag][key] = sortClusterMembersByKey(clusterTag, key);
                    })
                })
            };
            const generateStatisticsFromClusters = () => {
            ...
            const generateCategoricalOrdinationMetrics = () => {
                const generateQuantityInformation = () => {
                    const calculateQuantityDataStructures = (tag, key) => {
                            const calculateClusterQuantity = () => {
                                this.domain[key].forEach( category => {
                                    ...
                                    this.quantityOfClusterMembersEntriesThatPassOn[tag][key][category] = sum;
                                });
                            };
                            const calculateTotalQuantity = () => {
                                let sum = 0;
                                this.domain[key].forEach(category => {
                                    // TODO corrigir isso aqui a a soma ta diferente
                                    // console.log(this.quantityOfClusterMembersEntriesThatPassOn);
                                    // console.log(tag,key,category);
                                    sum += this.quantityOfClusterMembersEntriesThatPassOn[tag][key][category];
                                    this.totalQuantityOfEntriesThatPassOn[key][category] = sum;
                                });

                            };

                            calculateClusterQuantity();
                            calculateTotalQuantity();
                        };

                        this.quantityOfClusterMembersEntriesThatPassOn = {};
                        this.totalQuantityOfEntriesThatPassOn = {};
                        this.clusterTags.forEach( tag => {
                            this.quantityOfClusterMembersEntriesThatPassOn[tag] = {};
                            this.categoricalKeys.forEach( key => {
                                this.quantityOfClusterMembersEntriesThatPassOn[tag][key]= {};
                                this.totalQuantityOfEntriesThatPassOn[key] = {};
                                calculateQuantityDataStructures(tag, key);
                            })
                        });
                    };
                const generateOrdinationScales = () => {};

                generateQuantityInformation();
                generateOrdinationScales();
                console.log(this.quantityOfClusterMembersEntriesThatPassOn);
            };

            generateClusters();
            generateSortedClusters();
            generateStatisticsFromClusters();
            generateCategoricalOrdinationMetrics();
        };
        const updateScales = () => {
        ...
        this.axis = this.overlay.selectAll(".axis")
            ...
            .each(function(d) {
                if(self.domainType[d] !== 'Categorical') d3.select(this).call(axis.scale(y_axes[d]));
                    ...
                    let size = self.y[d](self.domain[d][0]) - self.y[d](self.domain[d][1]);
                    let PADDING = size/50;
                    self.domain[d].forEach((category, index) => {
                        let g = d3.select(this).append('g').attr('class', 'categoricalAxis')
                            .attr('transform','translate(0,'+index*size+')');

                        let s = d3.scale.linear().domain([0,1]).range([0, size-PADDING]);
...
var vis = {
    ...
    "ParallelBundling": ParallelBundling
};